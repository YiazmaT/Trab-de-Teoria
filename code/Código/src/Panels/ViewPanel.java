...
public class ViewPanel extends JPanel {
    ...
    public void addTrans(Estado source, Estado target, String parametro, String saida, String direção) {
        ...
        if (source != target) {
            trans.add(new TransicaoReta(source, target, parametro, saida, direção));
        } else {
            trans.add(new TransicaoAuto(source, target, parametro, saida, direção));
        }
    }

    public void addTrans(Estado source, Estado target, String[] a, String[] b, String[] indices) {
        for (int i = trans.size() - 1; i >= 0; i--) {
            Transicao t = trans.get(i);
            if (t.sameSourceTarget(source, target)) {
                t.addCondicao(a, b, indices);
                return;
            }
            if (t.opositeSourceTarget(source, target) && t.getTipo() == Transicao.TIPO_NORMAL) {
                trans.set(i, new TransicaoArco(t, Transicao.TIPO_CIMA));
                trans.add(new TransicaoArco(source, target, a, Transicao.TIPO_BAIXO, b, indices));
                return;
            }

        }
        if (source != target) {
            trans.add(new TransicaoReta(source, target, a, b, indices));
        } else {
            trans.add(new TransicaoAuto(source, target, a, b, indices));
        }
    }
    ...
    public void removerEntidade(int x, int y) {
        ...
        for (int i = trans.size() - 1; i >= 0; i--) {
            ...
            if (retorno == Transicao.NAO_REMOVE) {
                continue;
            }
            if (retorno == Transicao.REMOVE_TRANSICAO) {
                if (trans.get(i).getTipo() == Transicao.TIPO_BAIXO || trans.get(i).getTipo() == Transicao.TIPO_CIMA) {

                    for (int j = 0; j < trans.size(); j++) {
                        Transicao t = trans.get(j);
                        if (t.opositeSourceTarget(trans.get(i).getSource(), trans.get(i).getTarget())) {
                            trans.set(j, new TransicaoReta(t));
                        }
                    }
    ...
    public boolean verificaEstados(boolean needFinal) {
        ...
        if (hasInicial == true && hasFinal == true) {
            return true;
        }
        return false;
    }
